...
> The build automatically downloads `AmpLockerLinux.zip` from Audio Assault's
> S3 distribution server. A valid Amp Locker licence is required at runtime;
> the app phones home for activation.
...
- `curl` and `unzip` (for downloading and extracting version info)
...
The upstream `AmpLockerLinux.zip` is downloaded automatically from Audio
Assault's S3 server during the build process.
...
The build automatically downloads the zip from S3 (if not cached locally),
extracts the version from `moduleinfo.json`, and updates the metainfo before
building.
...
| `AmpLockerLinux.zip`                       | Upstream blob (auto-downloaded, cached locally; **not** in VCS) |
